...
export class MobileControls {
    constructor(keys) {
        ...
        // Determine mobile slightly more robustly
    ...
    handleTouchStart(e) {
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
            ...
            if (this.isTouchInsideElement(touchX, touchY, this.chatContainer) ||
                ...
                continue; // IMPORTANT: Skip joystick assignment for this touch
            }

            // Assign touch to available joystick if outside UI elements
            if (touchX < window.innerWidth / 2 && this.leftTouchId === null) {
                this.leftTouchId = touch.identifier; this.leftStartPos = { x: touchX, y: touchY };
                this.leftJoystick.style.display = 'block';
                this.leftJoystick.style.left = `${touchX - 60}px`; this.leftJoystick.style.top = `${touchY - 60}px`;
                this.leftJoystickHandle.style.transform = 'translate(-50%, -50%)';
                console.log(`Left joystick activated: ID ${this.leftTouchId}`);
                preventDefault = true; // Mark that this touch activated a joystick

            } else if (touchX >= window.innerWidth / 2 && this.rightTouchId === null) {
                this.rightTouchId = touch.identifier; this.rightStartPos = { x: touchX, y: touchY };
                this.rightJoystick.style.display = 'block';
                this.rightJoystick.style.left = `${touchX - 60}px`; this.rightJoystick.style.top = `${touchY - 60}px`;
                this.rightJoystickHandle.style.transform = 'translate(-50%, -50%)';
                console.log(`Right joystick activated: ID ${this.rightTouchId}`);
                preventDefault = true; // Mark that this touch activated a joystick
            }

            // Prevent default browser action (like scrolling) ONLY if this touch activated a joystick
            if (preventDefault) {
                 console.log(`Preventing default for touch ID: ${touch.identifier}`);
                 e.preventDefault();
            }
        }
    }
    ...
    handleTouchMove(e) {
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
            ...
            if(touch.identifier === this.leftTouchId || touch.identifier === this.rightTouchId) {
                 isJoystickTouchMoving = true; break;
            }
        }
        // Prevent default scrolling ONLY if one of the moved touches is controlling a joystick
        if(isJoystickTouchMoving) e.preventDefault(); else return;
        ...
        for (let i = 0; i < e.changedTouches.length; i++) {
            ...
            if (touch.identifier === this.leftTouchId) {
                let deltaX = touch.clientX - this.leftStartPos.x; let deltaY = touch.clientY - this.leftStartPos.y;
                const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
                if (distance > maxDistance) { const angle = Math.atan2(deltaY, deltaX); deltaX = Math.cos(angle) * maxDistance; deltaY = Math.sin(angle) * maxDistance; }
                this.leftJoystickHandle.style.transform = `translate(calc(-50% + ${deltaX}px), calc(-50% + ${deltaY}px))`;
                const nX = deltaX / maxDistance; const nY = deltaY / maxDistance;
                this.keys.arrowup = nY < -0.3; this.keys.arrowdown = nY > 0.3; this.keys.arrowleft = nX < -0.3; this.keys.arrowright = nX > 0.3;
            }
            if (touch.identifier === this.rightTouchId) {
                let deltaX = touch.clientX - this.rightStartPos.x; let deltaY = touch.clientY - this.rightStartPos.y;
                const distance = Math.sqrt(deltaX * deltaX + deltaY * deltaY);
                if (distance > maxDistance) { const angle = Math.atan2(deltaY, deltaX); deltaX = Math.cos(angle) * maxDistance; deltaY = Math.sin(angle) * maxDistance; }
                this.rightJoystickHandle.style.transform = `translate(calc(-50% + ${deltaX}px), calc(-50% + ${deltaY}px))`;
                const nX = deltaX / maxDistance; const nY = deltaY / maxDistance;
                this.keys.a = nX < -0.3; this.keys.d = nX > 0.3; this.keys.w = nY < -0.3; this.keys.s = nY > 0.3;
